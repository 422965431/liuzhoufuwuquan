...
namespace CouponMall.Services
{
    ...
    public partial class MallAdminGroups
    {
        ...
        {
            HashSet<string> actionHashSet = CouponMall.Core.BMACache.Get(CacheKeys.MALL_MALLADMINGROUP_ACTIONHASHSET + mallAGid) as HashSet<string>;
            if (actionHashSet == null)
            {
                ...
                if (mallAdminGroupInfo != null)
                {
                    ...
                    CouponMall.Core.BMACache.Insert(CacheKeys.MALL_MALLADMINGROUP_ACTIONHASHSET + mallAGid, actionHashSet);
                }
            }
        ...
        {
            MallAdminGroupInfo[] mallAdminGroupList = CouponMall.Core.BMACache.Get(CacheKeys.MALL_MALLADMINGROUP_LIST) as MallAdminGroupInfo[];
            if (mallAdminGroupList == null)
            {
                mallAdminGroupList = CouponMall.Data.MallAdminGroups.GetMallAdminGroupList();
                CouponMall.Core.BMACache.Insert(CacheKeys.MALL_MALLADMINGROUP_LIST, mallAdminGroupList);
            }
        ...
        public static void CreateMallAdminGroup(MallAdminGroupInfo mallAdminGroupInfo)
        {
            ...
            int mallAGid = CouponMall.Data.MallAdminGroups.CreateMallAdminGroup(mallAdminGroupInfo);
            if (mallAGid > 0)
            {
                CouponMall.Core.BMACache.Remove(CacheKeys.MALL_MALLADMINGROUP_LIST);
        ...
        public static int DeleteMallAdminGroupById(int mallAGid)
        {
            ...
            if (mallAGid > 0)
            {
                CouponMall.Data.MallAdminGroups.DeleteMallAdminGroupById(mallAGid);
                CouponMall.Core.BMACache.Remove(CacheKeys.MALL_MALLADMINGROUP_ACTIONHASHSET + mallAGid);
                CouponMall.Core.BMACache.Remove(CacheKeys.MALL_MALLADMINGROUP_LIST);
        ...
        {
            mallAdminGroupInfo.ActionList = mallAdminGroupInfo.ActionList.ToLower();
            CouponMall.Data.MallAdminGroups.UpdateMallAdminGroup(mallAdminGroupInfo);
            CouponMall.Core.BMACache.Remove(CacheKeys.MALL_MALLADMINGROUP_ACTIONHASHSET + mallAdminGroupInfo.MallAGid);
            CouponMall.Core.BMACache.Remove(CacheKeys.MALL_MALLADMINGROUP_LIST);
            WriteMallAdminNavMenuCache(mallAdminGroupInfo);
        }
        ...
        private static void WriteMallAdminNavMenuCache(MallAdminGroupInfo mallAdminGroupInfo)
        {
            ...
            if (mallAdminGroupActionHashSet.Contains("order"))
            {
                ...
                menu.AppendFormat("{0}\"title\":\"售后服务\",\"url\":\"/malladmin/order/afterservicelist\"{1},", "{", "}");
