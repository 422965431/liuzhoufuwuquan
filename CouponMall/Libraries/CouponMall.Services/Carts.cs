...
namespace CouponMall.Services
{
    ...
    public partial class Carts
    {
        ...
        /// <param name="orderProductList">订单商品列表</param>
        /// <returns></returns>
        public static StoreCartInfo TidyStoreOrderProductList(string[] selectedCartItemKeyList, List<OrderProductInfo> orderProductList)
        {
            //声明一个店铺购物车
                ...
                if (orderProductInfo.Type == 0)//当商品是普通订单商品时
                {
                    if (orderProductInfo.ExtCode4 > 0)//满赠订单商品处理
                    {
                        ...
                        if (fullSendPromotionInfo != null)
                        {
                            ...
                            if (cartProductInfo1.IsSelected)
                                mainProductAmount += orderProductInfo.DiscountPrice * orderProductInfo.BuyCount;
                            ...
                            for (int j = 0; j < count; j++)
                            {
                                ...
                                if (item != null && item.Type == 0 && item.ExtCode4 == orderProductInfo.ExtCode4)
                                {
                                    ...
                                    if (cartProductInfo2.IsSelected)
                                        mainProductAmount += item.DiscountPrice * item.BuyCount;
                                    orderProductList[j] = null;
                            ...
                            cartFullSendInfo.MainProductAmount = mainProductAmount;

                            storeCartInfo.CartFullSendList.Add(cartFullSendInfo);
                        }
                        else//当满赠促销活动不存在时，按照没有满赠促销商品处理
                        {
                            ...
                            storeCartInfo.CartProductList.Add(cartProductInfo1);
                            ...
                            for (int j = 0; j < count; j++)
                            {
                                ...
                                if (item != null && item.Type == 0 && item.ExtCode4 == orderProductInfo.ExtCode4)
                                {
                                    ...
                                    storeCartInfo.CartProductList.Add(cartProductInfo2);
                                }
                            }
                    ...
                    else if (orderProductInfo.ExtCode5 > 0)//满减订单商品处理
                    {
                        ...
                        if (fullCutPromotionInfo != null)
                        {
                            ...
                            cartFullCutInfo.ProductAmount = selectedFullCutCartProductAmount;

                            storeCartInfo.CartFullCutList.Add(cartFullCutInfo);
                        }
                    ...
                    else//非满赠和满减订单商品处理
                    {
                        ...
                        storeCartInfo.CartProductList.Add(cartProductInfo);
                ...
                else if (orderProductInfo.Type == 3)//当商品是套装商品时
                {
                    ...
                    for (int j = 0; j < count; j++)
                    {
                        ...
                        if (item != null && item.Type == 3 && item.ExtCode1 == orderProductInfo.ExtCode1)
                        {
                            ...
                            productAmount += item.ShopPrice * item.RealCount;
                    ...
                    cartSuitInfo.ProductAmount = productAmount;
                    cartSuitInfo.Discount = cartSuitInfo.ProductAmount - cartSuitInfo.SuitAmount;
                    cartSuitInfo.CartProductList = cartProductList;

                    storeCartInfo.CartSuitList.Add(cartSuitInfo);
            ...
            foreach (CartProductInfo cartProductInfo in storeCartInfo.CartProductList)
            {
                ...
                {
                    storeCartInfo.SelectedOrderProductList.Add(cartProductInfo.OrderProductInfo);
                    storeCartInfo.SelectedOrderProductList.AddRange(cartProductInfo.GiftList);
                }
                else
                {
                    storeCartInfo.RemainedOrderProductList.Add(cartProductInfo.OrderProductInfo);
                    storeCartInfo.RemainedOrderProductList.AddRange(cartProductInfo.GiftList);
                }
            }
            ...
            foreach (CartFullSendInfo cartFullSendInfo in storeCartInfo.CartFullSendList)
            {
                if (cartFullSendInfo.FullSendMinorOrderProductInfo != null)
                {
                    ...
                    {
                        storeCartInfo.SelectedOrderProductList.Add(cartFullSendInfo.FullSendMinorOrderProductInfo);
                    }
                    else
                    {
                        storeCartInfo.RemainedOrderProductList.Add(cartFullSendInfo.FullSendMinorOrderProductInfo);
                    }
                }
            ...
            storeCartInfo.IsSelected = storeCartInfo.SelectedOrderProductList.Count > 0;
        ...
        /// <param name="orderProductList">订单商品列表</param>
        /// <returns></returns>
        public static List<StoreCartInfo> TidyMallOrderProductList(string[] selectedCartItemKeyList, List<OrderProductInfo> orderProductList)
        {
            ...
            foreach (int storeId in storeIdList.Distinct<int>())
            {
                ...
                StoreCartInfo storeCartInfo = TidyStoreOrderProductList(selectedCartItemKeyList, storeOrderProductList);
                storeCartList.Add(storeCartInfo);
            }
        ...
        /// 汇总满减
        /// </summary>
        /// <param name="storeCartInfo">店铺购物车</param>
        /// <returns></returns>
        public static int SumFullCut(StoreCartInfo storeCartInfo)
        {
            //满减
            int fullCut = 0;
            foreach (CartFullCutInfo cartFullCutInfo in storeCartInfo.CartFullCutList)
        ...
        public static int SumMallCartFullCut(List<StoreCartInfo> storeCartList)
        {
            int fullCut = 0;
            foreach (StoreCartInfo storeCartInfo in storeCartList)
                fullCut += SumFullCut(storeCartInfo);
            return fullCut;
        }
        ...
        /// <param name="storeCartList">店铺购物车列表</param>
        /// <returns></returns>
        public static int SumSelectedStoreCount(List<StoreCartInfo> storeCartList)
        {
            int count = 0;
            foreach (StoreCartInfo storeCartInfo in storeCartList)
            {
                if (storeCartInfo.IsSelected)
                    ++count;
            }
            return count;
        }
