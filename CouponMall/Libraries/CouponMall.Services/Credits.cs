...
namespace CouponMall.Services
{
    ...
    public partial class Credits
    {
        ...
        /// 支付积分价格(单位为100个)
        /// </summary>
        public static int PayCreditPrice
        {
            get { return _creditconfiginfo.PayCreditPrice; }
        }
        ...
        public static void SendLoginCredits(ref PartUserInfo partUserInfo, DateTime loginTime, DateTime oldTime, out DateTime newTime)
        {
            ...
            if (_creditconfiginfo.LoginPayCredits > 0 || _creditconfiginfo.LoginRankCredits > 0)
            {
                ...
                if (!IsSendTodayLoginCredit(partUserInfo.Uid, DateTime.Now))
                {
                    ...
                    partUserInfo.UserRid = UserRanks.GetUserRankByCredits(partUserInfo.RankCredits).UserRid;
        ...
        public static void SendRegisterCredits(ref UserInfo userInfo, DateTime registerTime)
        {
            if (_creditconfiginfo.RegisterPayCredits > 0 || _creditconfiginfo.RegisterRankCredits > 0)
            {
                ...
                userInfo.UserRid = UserRanks.GetUserRankByCredits(userInfo.RankCredits).UserRid;
        ...
        public static void SendReceiveOrderCredits(ref PartUserInfo partUserInfo, OrderInfo orderInfo, List<OrderProductInfo> orderProductList, DateTime receiveTime)
        {
            if (_creditconfiginfo.ReceiveOrderPayCredits > 0 || _creditconfiginfo.ReceiveOrderRankCredits > 0)
            {
                int surplusPayCredits = GetDaySurplusPayCredits(partUserInfo.Uid, receiveTime.Date);
                int surplusRankCredits = GetDaySurplusRankCredits(partUserInfo.Uid, receiveTime.Date);
                if (surplusPayCredits == 0 && surplusRankCredits == 0)
                    return;

                int payCredits = 0;
                int rankCredits = 0;
                int tempPayCredits = (int)Math.Floor(orderInfo.OrderAmount * _creditconfiginfo.ReceiveOrderPayCredits / 100);
                int tempRankCredits = (int)Math.Floor(orderInfo.OrderAmount * _creditconfiginfo.ReceiveOrderRankCredits / 100);
                if (surplusPayCredits > 0)
        ...
        {
            CouponMall.Data.Credits.SendCredits(userRid, creditLogInfo);
        }
        ...
        {
            CouponMall.Data.Credits.SendPayCredits(creditLogInfo);
        }
        ...
        {
            return CouponMall.Data.Credits.GetPayCreditLogList(uid, type, pageSize, pageNumber);
        }
        ...
        {
            return CouponMall.Data.Credits.GetPayCreditLogCount(uid, type);
        }
        ...
        {
            return CouponMall.Data.Credits.GetTodaySendPayCredits(uid, today);
        }
        ...
        {
            return CouponMall.Data.Credits.GetTodaySendRankCredits(uid, today);
        }
        ...
        {
            return CouponMall.Data.Credits.IsSendTodayLoginCredit(uid, today);
        }
        ...
        {
            return CouponMall.Data.Credits.IsSendCompleteUserInfoCredit(uid);
        }
        ...
        {
            return CouponMall.Data.Credits.GetUserOrderSendCredits(oid);
        }
    }
}
