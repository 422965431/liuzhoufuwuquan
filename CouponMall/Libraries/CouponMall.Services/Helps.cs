...
namespace CouponMall.Services
{
    ...
    public partial class Helps
    {
        ...
        {
            List<HelpInfo> helpTree = CouponMall.Core.BMACache.Get(CacheKeys.MALL_HELP_LIST) as List<HelpInfo>;

            if (helpTree == null)
            {
                helpTree = new List<HelpInfo>();
                List<HelpInfo> helpList = CouponMall.Data.Helps.GetHelpList();

                CreateHelpTree(helpList, helpTree, 0);
                CouponMall.Core.BMACache.Insert(CacheKeys.MALL_HELP_LIST, helpTree);
            }
            return helpTree;
        }
