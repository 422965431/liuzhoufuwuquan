...
namespace CouponMall.Services
{
    ...
    public partial class Navs
    {
        ...
        {
            List<NavInfo> navTree = CouponMall.Core.BMACache.Get(CacheKeys.MALL_NAV_LIST) as List<NavInfo>;

            if (navTree == null)
            {
                navTree = new List<NavInfo>();
                List<NavInfo> navList = CouponMall.Data.Navs.GetNavList();

                CreateNavTree(navList, navTree, 0);
                CouponMall.Core.BMACache.Insert(CacheKeys.MALL_NAV_LIST, navTree);
            }
            return navTree;
        }
        ...
        {
            List<NavInfo> navList = CouponMall.Core.BMACache.Get(CacheKeys.MALL_NAV_MAINLIST) as List<NavInfo>;

            if (navList == null)
            {
                navList = new List<NavInfo>();
                foreach (NavInfo navInfo in CouponMall.Data.Navs.GetNavList())
                {
                    if (navInfo.Pid == 0)
                        navList.Add(navInfo);
                }
                CouponMall.Core.BMACache.Insert(CacheKeys.MALL_NAV_MAINLIST, navList);
            }
            return navList;
        }
