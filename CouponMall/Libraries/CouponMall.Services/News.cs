...
namespace CouponMall.Services
{
    ...
    public partial class News
    {
        ...
        {
            List<NewsTypeInfo> newsTypeList = CouponMall.Core.BMACache.Get(CacheKeys.MALL_NEWSTYPE_LIST) as List<NewsTypeInfo>;
            if (newsTypeList == null)
            {
                newsTypeList = CouponMall.Data.News.GetNewsTypeList();
                CouponMall.Core.BMACache.Insert(CacheKeys.MALL_NEWSTYPE_LIST, newsTypeList);
            }
            return newsTypeList;
        }
        ...
        {
            if (newsId > 0)
                return CouponMall.Data.News.GetNewsById(newsId);
            return null;
        }
        ...
        {
            DataTable newsList = CouponMall.Core.BMACache.Get(CacheKeys.MALL_NEWS_HOMELIST + newsTypeId) as DataTable;
            if (newsList == null)
            {
                newsList = CouponMall.Data.News.GetHomeNewsList(newsTypeId, count);
                CouponMall.Core.BMACache.Insert(CacheKeys.MALL_NEWS_HOMELIST + newsTypeId, newsList);
            }
            return newsList;
        }
        ...
        {
            return CouponMall.Data.News.GetNewsListCondition(newsTypeId, title);
        }
        ...
        {
            return CouponMall.Data.News.GetNewsList(pageSize, pageNumber, condition);
        }
        ...
        {
            return CouponMall.Data.News.GetNewsCount(condition);
        }
    }
}
