...
namespace CouponMall.Services
{
    ...
    public partial class UserRanks
    {
        ...
        {
            List<UserRankInfo> userRankList = CouponMall.Core.BMACache.Get(CacheKeys.MALL_USERRANK_LIST) as List<UserRankInfo>;
            if (userRankList == null)
            {
                userRankList = CouponMall.Data.UserRanks.GetUserRankList();
                CouponMall.Core.BMACache.Insert(CacheKeys.MALL_USERRANK_LIST, userRankList);
            }
            return userRankList;
        }
