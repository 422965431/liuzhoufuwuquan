...
namespace CouponMall.PayPlugin.Alipay
{
    /// <summary>
    ...
    public class AlipayNotify
    {
        ...
        /// <param name="sign">支付宝生成的签名结果</param>
        /// <returns>验证结果</returns>
        public static bool AppVerify(SortedDictionary<string, string> inputPara, string notify_id, string sign, string signType, string publicKey, string appInputCharset, string appVeryfyUrl, string partner)
        {
            //获取返回时的签名验证结果
            bool isSign = GetAppSignVeryfy(inputPara, sign, signType, publicKey, appInputCharset);
            //获取是否是支付宝服务器发来的请求的验证结果
            string responseTxt = "true";
            if (notify_id != null && notify_id != "") { responseTxt = GetAppResponseTxt(notify_id, appVeryfyUrl, partner); }
        ...
        /// <param name="sign">对比的签名结果</param>
        /// <returns>签名验证结果</returns>
        private static bool GetAppSignVeryfy(SortedDictionary<string, string> inputPara, string sign, string signType, string publicKey, string appInputCharset)
        {
            Dictionary<string, string> sPara = new Dictionary<string, string>();
            ...
            if (sign != null && sign != "")
            {
                ...
                {
                    case "RSA":
                        isSgin = AlipayRSAFromPkcs8.verify(preSignStr, sign, publicKey, appInputCharset);
                        break;
                    default:
                        break;
                }
        ...
        /// <param name="notify_id">通知验证ID</param>
        /// <returns>验证结果</returns>
        private static string GetAppResponseTxt(string notify_id, string appVeryfyUrl, string partner)
        {
            string veryfy_url = appVeryfyUrl + "partner=" + partner + "&notify_id=" + notify_id;
