...
namespace CouponMall.Web.MallAdmin.Controllers
{
    ...
    /// 商城后台商城帮助控制器类
    /// </summary>
    public partial class HelpController : BaseMallAdminController
    {
        /// <summary>
        /// 商城帮助列表
        /// </summary>
        ...
        public ActionResult Del(int id = -1)
        {
            ...
            AddMallAdminLog("删除帮助", "删除帮助,帮助ID为:" + id);
        ...
        public ActionResult UpdateHelpDisplayOrder(int id = -1, int displayOrder = 0)
        {
            ...
            AddMallAdminLog("修改帮助的顺序", "修改帮助的顺序,帮助ID:" + id);
            if (result)
                return Content("1");
            else
                return Content("0");
        ...
        private void Load()
        {
            List<SelectListItem> helpCategoryList = new List<SelectListItem>();
            helpCategoryList.Add(new SelectListItem() { Text = "请选择分类", Value = "0" });
            foreach (HelpInfo helpInfo in AdminHelps.GetHelpCategoryList())
            {
                helpCategoryList.Add(new SelectListItem() { Text = helpInfo.Title, Value = helpInfo.Id.ToString() });
            }
            ViewData["helpCategoryList"] = helpCategoryList;
            ViewData["referer"] = MallUtils.GetMallAdminRefererCookie();
        }
    }
}
