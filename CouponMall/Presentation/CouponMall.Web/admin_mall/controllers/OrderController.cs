...
namespace CouponMall.Web.MallAdmin.Controllers
{
    ...
    public partial class OrderController : BaseMallAdminController
    {
        ...
        /// <param name="pageNumber">当前页数</param>
        /// <returns></returns>
        public ActionResult AfterServiceList(string storeName, string accountName, string applyStartTime, string applyEndTime, int storeId = 0, int oid = 0, int state = -1, int type = -1, int pageSize = 15, int pageNumber = 1)
        {
            //获取用户id
            int uid = Users.GetUidByAccountName(accountName);

            string condition = AdminOrderAfterServices.GetOrderProductAfterServiceListCondition(storeId, uid, oid, state, type, applyStartTime, applyEndTime);
            ...
            MallUtils.SetAdminRefererCookie(string.Format("{0}?pageNumber={1}&pageSize={2}&storeId={3}&storeName={4}&oid={5}&AccountName={6}&state={7}&type={8}&applyStartTime={9}&applyEndTime={10}",
                                                          Url.Action("afterservicelist"),
                                                          pageModel.PageNumber, pageModel.PageSize,
                                                          storeId, storeName, oid, accountName, state, type, applyStartTime, applyEndTime));
