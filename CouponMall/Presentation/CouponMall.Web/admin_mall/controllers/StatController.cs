...
namespace CouponMall.Web.MallAdmin.Controllers
{
    ...
    public partial class StatController : BaseMallAdminController
    {
        ...
        public ActionResult OnlineUserList(int provinceId = -1, int cityId = -1, int regionId = -1, int pageNumber = 1, int pageSize = 15)
        {
            ...
            MallUtils.SetAdminRefererCookie(string.Format("{0}?pageNumber={1}&pageSize={2}&provinceId={3}&cityId={4}&regionId={5}",
        ...
        public ActionResult RegionStat()
        {
            ...
            model.RegionStat = PVStats.GetProvinceRegionStat(0);
        ...
        {
            PageModel pageModel = new PageModel(pageSize, pageNumber, AdminOrders.GetSaleProductCount(0, startTime, endTime, orderState));
            ...
            {
                PageModel = pageModel,
                SaleProductList = AdminOrders.GetSaleProductList(pageModel.PageSize, pageModel.PageNumber, 0, startTime, endTime, orderState),
                StartTime = startTime,
                EndTime = endTime,
        ...
        /// <param name="storeId">店铺id</param>
        /// <returns></returns>
        public ActionResult OrderStat(string storeName, string startTime, string endTime, int statType = 0, int storeId = -1)
        {
            ...
            model.StoreName = string.IsNullOrWhiteSpace(storeName) ? "全部店铺" : storeName;
            model.StartTime = startTime;
            model.EndTime = endTime;
            model.StatItemList = AdminOrders.GetOrderStat(statType, storeId, startTime, endTime);
