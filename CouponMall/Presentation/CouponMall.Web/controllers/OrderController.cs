        ...
        public ActionResult SubmitOrder()
        {
            lock (_locker)
            {
                ...
                foreach (StoreCartInfo storeCartInfo in storeCartList)
                {
                    ...
                    OrderInfo orderInfo = Orders.CreateOrder(WorkContext.PartUserInfo, storeCartInfo.StoreInfo, storeCartInfo.SelectedOrderProductList, storeSinglePromotionList, fullShipAddressInfo, payMode == 0 ? 0 : 1, ref payCreditCount, storeCouponList, storeFullCut, buyerRemark, bestTime, WorkContext.IP);
                    if (orderInfo != null)
                    {
                        //创建订单处理
