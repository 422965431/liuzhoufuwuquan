...
namespace CouponMall.Web.Mobile.Controllers
{
    ...
    public partial class OrderController : BaseMobileController
    {
        ...
        public ActionResult ConfirmOrder()
        {
            ...
            {
                model.AllProductAmount += Carts.SumOrderProductAmount(item.SelectedOrderProductList);
                model.AllFullCut += Carts.SumFullCut(item);
                model.AllShipFee += model.DefaultFullShipAddressInfo != null ? Orders.GetShipFee(model.DefaultFullShipAddressInfo.ProvinceId, model.DefaultFullShipAddressInfo.CityId, item.SelectedOrderProductList) : 0;
                model.AllTotalCount += Carts.SumOrderProductCount(item.SelectedOrderProductList);
                model.AllTotalWeight += Carts.SumOrderProductWeight(item.SelectedOrderProductList);
            }
        ...
        public ActionResult SubmitOrder()
        {
            lock (_locker)
            {
                ...
                foreach (StoreCartInfo storeCartInfo in storeCartList)
                {
                    ...
                    OrderInfo orderInfo = Orders.CreateOrder(WorkContext.PartUserInfo, storeCartInfo.StoreInfo, storeCartInfo.SelectedOrderProductList, storeSinglePromotionList, fullShipAddressInfo, payMode == 0 ? 0 : 1, ref payCreditCount, storeCouponList, storeFullCut, "", new DateTime(1970, 1, 1), WorkContext.IP);
                    if (orderInfo != null)
                    {
        ...
        public ActionResult PayShow()
        {
            ...
            if (orderList.Count < 1 || allPayMoney == 0M)
                return RedirectToAction("index", "home");
