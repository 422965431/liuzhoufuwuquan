...
namespace CouponMall.IPSeekStrategy.IPFile
{
    /// <summary>
    /// 基于IP纯真库的IP查找策略
    /// </summary>
    public partial class IPSeekStrategy : IIPSeekStrategy
    {
        ...
        /// <returns></returns>
        private IPLocation SearchLocation(string ip)
        {
            lock (_locker)
            {
                if (_state)
