...
namespace CouponMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        /// <returns></returns>
        public string AdminGetProductListCondition(int storeId, int storeCid, string productName, int cateId, int brandId, int state)
        {
            ...
            if (storeCid > 0)
                condition.AppendFormat(" AND [storecid] = {0} ", storeCid);
        ...
        public DataTable AdminGetProductList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[storeid],[temp1].[name] AS [pname],[temp1].[shopprice],[temp1].[marketprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp2].[number],[temp3].[name] AS [sname] FROM (SELECT TOP {0} [pid],[storeid],[name],[shopprice],[marketprice],[state],[isbest],[ishot],[isnew],[displayorder] FROM [{1}products] ORDER BY [displayorder] DESC,[pid] DESC) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);
        ...
        public int CreateProduct(ProductInfo productInfo)
        {
            ...
            string commandText = string.Format(@"INSERT INTO [{0}products]([psn],[cateid],[brandid],[storeid],[storecid],[storestid],[skugid],[name],[shopprice],[marketprice],[costprice],[state],[isbest],[ishot],[isnew],
                                                ...
                                                VALUES(@psn,@cateid,@brandid,@storeid,@storecid,@storestid,@skugid,@name,@shopprice,@marketprice,@costprice,@state,@isbest,@ishot,@isnew,
                                                @weight,@displayorder,@showimg,@salecount,@visitcount,@reviewcount,@star1,@star2,@star3,@star4,@star5,@addtime,@description);
        ...
        /// <param name="sortDirection">排序方向</param>
        /// <returns></returns>
        public IDataReader GetStoreClassProductList(int pageSize, int pageNumber, int storeCid, int startPrice, int endPrice, int sortColumn, int sortDirection)
        {
            StringBuilder commandText = new StringBuilder();

            if (pageNumber == 1)
            {
                commandText.AppendFormat("SELECT TOP {1} [pid],[psn],[cateid],[brandid],[storeid],[storecid],[storestid],[skugid],[name],[shopprice],[marketprice],[costprice],[state],[isbest],[ishot],[isnew],[displayorder],[weight],[showimg],[salecount],[visitcount],[reviewcount],[star1],[star2],[star3],[star4],[star5],[addtime] FROM [{0}products]", RDBSHelper.RDBSTablePre, pageSize);

                commandText.AppendFormat(" WHERE [storecid]={0}", storeCid);
            ...
            else
            {
                ...
                commandText.Append(",[pid] DESC");
                commandText.AppendFormat(") AS [rowid],[pid],[psn],[cateid],[brandid],[storeid],[storecid],[storestid],[skugid],[name],[shopprice],[marketprice],[costprice],[state],[isbest],[ishot],[isnew],[displayorder],[weight],[showimg],[salecount],[visitcount],[reviewcount],[star1],[star2],[star3],[star4],[star5],[addtime] FROM [{0}products]", RDBSHelper.RDBSTablePre);
        ...
        /// <param name="endPrice">结束价格</param>
        /// <returns></returns>
        public int GetStoreClassProductCount(int storeCid, int startPrice, int endPrice)
        {
            StringBuilder commandText = new StringBuilder();

            commandText.AppendFormat("SELECT COUNT([pid]) FROM [{0}products]", RDBSHelper.RDBSTablePre);
        ...
        public IDataReader GetStoreTraitProductList(int count, int storeId, int storeCid, int trait)
        {
            DbParameter[] parms =  { 
                                        ...
                                        GenerateInParam("@trait", SqlDbType.TinyInt, 1, trait)
                                    };
            return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,
                                            string.Format("{0}getstoretraitproductlist", RDBSHelper.RDBSTablePre),
                                            parms);
        }
        ...
        public IDataReader GetStoreSaleProductList(int count, int storeId, int storeCid)
        {
            ...
            return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,
                                            string.Format("{0}getstoresaleproductlist", RDBSHelper.RDBSTablePre),
                                            parms);
        }
        ...
        /// <param name="pidList">商品id列表</param>
        /// <returns></returns>
        public DataTable GetStoreIdListByPid(string pidList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}products] WHERE [pid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               pidList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        /// <param name="storeId">店铺id</param>
        /// <returns></returns>
        public int AdminGetStoreProductCount(int storeId)
        {
            DbParameter[] parms =  {
                                     GenerateInParam("@storeid", SqlDbType.Int, 4, storeId)
                                    };
            string commandText = string.Format("SELECT COUNT([pid]) FROM [{0}products] WHERE [storeid]=@storeid",
                                                RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        /// <param name="storeCid">店铺分类id</param>
        /// <returns></returns>
        public int AdminGetStoreClassProductCount(int storeCid)
        {
            DbParameter[] parms =  {
                                     GenerateInParam("@storecid", SqlDbType.Int, 4, storeCid)
                                    };
            string commandText = string.Format("SELECT COUNT([pid]) FROM [{0}products] WHERE [storecid]=@storecid",
                                                RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        /// <returns></returns>
        public int AdminGetStoreShipTemplateProductCount(int storeSTid)
        {
            DbParameter[] parms =  {
                                     GenerateInParam("@storestid", SqlDbType.Int, 4, storeSTid)
                                    };
            string commandText = string.Format("SELECT COUNT([pid]) FROM [{0}products] WHERE [storestid]=@storestid",
                                                RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        public void CreateProductImage(ProductImageInfo productImageInfo)
        {
            ...
            commandText.AppendFormat("INSERT INTO [{0}productimages]([pid],[showimg],[ismain],[displayorder],[storeid]) VALUES(@pid,@showimg,@ismain,@displayorder,@storeid)", RDBSHelper.RDBSTablePre);
        ...
        /// <param name="productName">商品名称</param>
        /// <returns></returns>
        public DataTable GetTimeProductList(int pageSize, int pageNumber, int storeId, string productName)
        {
            string commandText;
            if (pageNumber == 1)
            {
                if (string.IsNullOrWhiteSpace(productName) || !SecureHelper.IsSafeSqlString(productName))
                    commandText = string.Format("SELECT [temp1].[recordid],[temp1].[pid],[temp1].[storeid],[temp1].[onsalestate],[temp1].[outsalestate],[temp1].[onsaletime],[temp1].[outsaletime],[temp2].[name] AS [pname],[temp3].[name] AS [sname] FROM (SELECT TOP {0} [recordid],[pid],[storeid],[onsalestate],[outsalestate],[onsaletime],[outsaletime] FROM [{1}timeproducts] {2} ORDER BY [recordid] DESC) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                storeId > 0 ? "WHERE [storeid]=" + storeId : "");

                else
                    commandText = string.Format("SELECT TOP {0} [temp1].[recordid],[temp1].[pid],[temp1].[storeid],[temp1].[onsalestate],[temp1].[outsalestate],[temp1].[onsaletime],[temp1].[outsaletime],[temp2].[name] AS [pname],[temp3].[name] AS [sname] FROM (SELECT [recordid],[pid],[storeid],[onsalestate],[outsalestate],[onsaletime],[outsaletime] FROM [{1}timeproducts]) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid] WHERE [temp2].[name] LIKE '{2}%' {3} ORDER BY [temp1].[recordid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                productName,
                                                storeId > 0 ? "AND [temp1].[storeid]=" + storeId : "");
            }
            else
            {
                if (string.IsNullOrWhiteSpace(productName) || !SecureHelper.IsSafeSqlString(productName))
                    commandText = string.Format("SELECT [temp1].[recordid],[temp1].[pid],[temp1].[storeid],[temp1].[onsalestate],[temp1].[outsalestate],[temp1].[onsaletime],[temp1].[outsaletime],[temp2].[name] AS [pname],[temp4].[name] AS [sname] FROM (SELECT TOP {0} [recordid],[pid],[storeid],[onsalestate],[outsalestate],[onsaletime],[outsaletime] FROM [{1}timeproducts] WHERE [recordid]<(SELECT MAX([recordid]) FROM (SELECT TOP {2} [recordid] FROM [{1}timeproducts] {3} ORDER BY [recordid] DESC) AS [temp3]) {4} ORDER BY [recordid] DESC) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp4] ON [temp1].[storeid]=[temp4].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                storeId > 0 ? "WHERE [storeid]=" + storeId : "",
                    ...
                    commandText = string.Format("SELECT [recordid],[pid],[storeid],[onsalestate],[outsalestate],[onsaletime],[outsaletime],[pname],[sname] FROM (SELECT TOP {0} [recordid],[pid],[storeid],[onsalestate],[outsalestate],[onsaletime],[outsaletime],[pname],[sname] FROM (SELECT TOP {2} [temp1].[recordid],[temp1].[pid],[temp1].[storeid],[temp1].[onsalestate],[temp1].[outsalestate],[temp1].[onsaletime],[temp1].[outsaletime],[temp2].[name] AS [pname],[temp5].[name] AS [sname] FROM [{1}timeproducts] AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp5] ON [temp1].[storeid]=[temp5].[storeid] WHERE [temp2].[name] LIKE '{3}%' {4} ORDER BY [temp1].[recordid] DESC) AS [temp3] ORDER BY [temp3].[recordid] ASC) AS [temp4] ORDER BY [temp4].[recordid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                productName,
        ...
        /// <param name="productName">商品名称</param>
        /// <returns></returns>
        public int GetTimeProductCount(int storeId, string productName)
        {
            string commandText;
            if (string.IsNullOrWhiteSpace(productName) || !SecureHelper.IsSafeSqlString(productName))
                commandText = string.Format("SELECT COUNT([recordid]) FROM [{0}timeproducts] {1}", RDBSHelper.RDBSTablePre, storeId > 0 ? "WHERE [storeid]=" + storeId : "");
            else
                commandText = string.Format("SELECT COUNT([temp1].[recordid]) FROM [{0}timeproducts] AS [temp1] INNER JOIN [{0}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] {2} AND [temp2].[name] LIKE '{1}%'", RDBSHelper.RDBSTablePre, productName, storeId > 0 ? "AND [temp1].[storeid]=" + storeId : "");
        ...
        public void ConsultProduct(int pid, int consultTypeId, int consultUid, int storeId, DateTime consultTime, string consultMessage, string consultNickName, string pName, string pShowImg, string consultIP)
        {
            DbParameter[] parms = {
                                        GenerateInParam("@pid", SqlDbType.Int,4,pid),
                                        GenerateInParam("@consulttypeid", SqlDbType.TinyInt,1,consultTypeId),
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public DataTable AdminGetProductConsultList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[consultid],[temp1].[pid],[temp1].[consulttypeid],[temp1].[state],[temp1].[consultuid],[temp1].[replyuid],[temp1].[storeid],[temp1].[consulttime],[temp1].[replytime],[temp1].[consultmessage],[temp1].[replymessage],[temp1].[consultnickname],[temp1].[replynickname],[temp1].[pname],[temp1].[pshowimg],[temp1].[consultip],[temp1].[replyip],[temp2].[name] AS [sname] FROM (SELECT TOP {0} {1} FROM [{2}productconsults] ORDER BY [consultid] DESC) AS [temp1] LEFT JOIN [{2}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                TableFields.PRODUCT_CONSULTS,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT [temp1].[consultid],[temp1].[pid],[temp1].[consulttypeid],[temp1].[state],[temp1].[consultuid],[temp1].[replyuid],[temp1].[storeid],[temp1].[consulttime],[temp1].[replytime],[temp1].[consultmessage],[temp1].[replymessage],[temp1].[consultnickname],[temp1].[replynickname],[temp1].[pname],[temp1].[pshowimg],[temp1].[consultip],[temp1].[replyip],[temp2].[name] AS [sname] FROM (SELECT TOP {0} {1} FROM [{2}productconsults] WHERE {3} ORDER BY [consultid] DESC) AS [temp1] LEFT JOIN [{2}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                TableFields.PRODUCT_CONSULTS,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[consultid],[temp1].[pid],[temp1].[consulttypeid],[temp1].[state],[temp1].[consultuid],[temp1].[replyuid],[temp1].[storeid],[temp1].[consulttime],[temp1].[replytime],[temp1].[consultmessage],[temp1].[replymessage],[temp1].[consultnickname],[temp1].[replynickname],[temp1].[pname],[temp1].[pshowimg],[temp1].[consultip],[temp1].[replyip],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY [consultid] DESC) AS [rowid],{0} FROM [{1}productconsults]) AS [temp] WHERE [temp].[rowid] BETWEEN {3} AND {2}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.PRODUCT_CONSULTS,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1);
                else
                    commandText = string.Format("SELECT [temp1].[consultid],[temp1].[pid],[temp1].[consulttypeid],[temp1].[state],[temp1].[consultuid],[temp1].[replyuid],[temp1].[storeid],[temp1].[consulttime],[temp1].[replytime],[temp1].[consultmessage],[temp1].[replymessage],[temp1].[consultnickname],[temp1].[replynickname],[temp1].[pname],[temp1].[pshowimg],[temp1].[consultip],[temp1].[replyip],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY [consultid] DESC) AS [rowid],{0} FROM [{1}productconsults] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {3} AND {2}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.PRODUCT_CONSULTS,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1,
        ...
        /// <returns></returns>
        public string AdminGetProductConsultListCondition(int consultTypeId, int storeId, int pid, int uid, string consultMessage, string consultStartTime, string consultEndTime)
        {
            StringBuilder condition = new StringBuilder();
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByConsultId(string consultIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}productconsults] WHERE [consultid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               consultIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public void ReviewProduct(ProductReviewInfo productReviewInfo)
        {
            DbParameter[] parms = {
                                    ...
                                    GenerateInParam("@storeid", SqlDbType.Int, 4, productReviewInfo.StoreId),
        ...
        public DataTable AdminGetProductReviewList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[storeid],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname],[temp3].[name] AS [sname] FROM (SELECT TOP {0} {2} FROM [{1}productreviews] ORDER BY [reviewid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.PRODUCT_REVIEWS);
                else
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[storeid],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname],[temp3].[name] AS [sname] FROM (SELECT TOP {0} {3} FROM [{1}productreviews] WHERE {2} ORDER BY [reviewid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                TableFields.PRODUCT_REVIEWS);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[storeid],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname],[temp3].[name] AS [sname] FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY [reviewid] DESC) AS [rowid],{0} FROM [{1}productreviews]) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid] WHERE [temp1].[rowid] BETWEEN {3} AND {2}",
                                                TableFields.PRODUCT_REVIEWS,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1);
                else
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[storeid],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname],[temp3].[name] AS [sname] FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY [reviewid] DESC) AS [rowid],{0} FROM [{1}productreviews] WHERE {4}) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid] WHERE [temp1].[rowid] BETWEEN {3} AND {2}",
                                                TableFields.PRODUCT_REVIEWS,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1,
        ...
        /// <returns></returns>
        public string AdminGetProductReviewListCondition(int storeId, int pid, string message, string startTime, string endTime)
        {
