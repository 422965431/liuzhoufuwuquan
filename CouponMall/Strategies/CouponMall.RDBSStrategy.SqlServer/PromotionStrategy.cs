...
namespace CouponMall.RDBSStrategy.SqlServer
{
    /// <summary>
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public void DeleteSinglePromotionById(string pmIdList)
        {
        ...
        /// <returns></returns>
        public DataTable AdminGetSinglePromotionList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[pid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name],[temp1].[discounttype],[temp2].[name] AS [pname],[temp2].[state] AS [pstate],[temp3].[name] AS [sname] FROM (SELECT TOP {0} [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[discounttype] FROM [{1}singlepromotions] ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[pid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name],[temp1].[discounttype],[temp2].[name] AS [pname],[temp2].[state] AS [pstate],[temp3].[name] AS [sname] FROM (SELECT TOP {0} [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[discounttype] FROM [{1}singlepromotions] WHERE {2} ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            }
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[pid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name],[temp1].[discounttype],[temp2].[name] AS [pname],[temp2].[state] AS [pstate],[temp3].[name] AS [sname] FROM (SELECT [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[discounttype] FROM (SELECT TOP {0} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],[pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[discounttype] FROM [{1}singlepromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                    ...
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[pid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name],[temp1].[discounttype],[temp2].[name] AS [pname],[temp2].[state] AS [pstate],[temp3].[name] AS [sname] FROM (SELECT [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[discounttype] FROM (SELECT TOP {0} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],[pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[discounttype] FROM [{1}singlepromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                pageSize * (pageNumber - 1) + 1,
                                                pageSize * pageNumber,
        ...
        /// <returns></returns>
        public string AdminGetSinglePromotionListCondition(int storeId, int pid, string promotionName, string promotionTime)
        {
            ...
            if (!string.IsNullOrWhiteSpace(promotionName) && SecureHelper.IsSafeSqlString(promotionName))
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListBySinglePmId(string pmIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}singlepromotions] WHERE [pmid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               pmIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public IDataReader AdminGetBuySendPromotionById(int pmId)
        {
            ...
            string commandText = string.Format("SELECT {1} FROM [{0}buysendpromotions] WHERE [pmid]=@pmid",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.BUYSEND_PROMOTIONS);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        {
            DbParameter[] parms = {
                                        GenerateInParam("@storeid", SqlDbType.Int, 4, buySendPromotionInfo.StoreId),
            ...
            string commandText = string.Format("INSERT INTO [{0}buysendpromotions]([storeid],[starttime],[endtime],[userranklower],[state],[name],[type],[buycount],[sendcount]) VALUES(@storeid,@starttime,@endtime,@userranklower,@state,@name,@type,@buycount,@sendcount)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public void UpdateBuySendPromotion(BuySendPromotionInfo buySendPromotionInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}buysendpromotions] SET [storeid]=@storeid,[starttime]=@starttime,[endtime]=@endtime,[userranklower]=@userranklower,[state]=@state,[name]=@name,[type]=@type,[buycount]=@buycount,[sendcount]=@sendcount WHERE [pmid]=@pmid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public DataTable AdminGetBuySendPromotionList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime],[temp1].[endtime],[temp1].[userranklower],[temp1].[state],[temp1].[name] AS [pname],[temp1].[type],[temp1].[buycount],[temp1].[sendcount],[temp2].[name] AS [sname] FROM (SELECT TOP {0} {2} FROM [{1}buysendpromotions] ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.BUYSEND_PROMOTIONS);

                else
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime],[temp1].[endtime],[temp1].[userranklower],[temp1].[state],[temp1].[name] AS [pname],[temp1].[type],[temp1].[buycount],[temp1].[sendcount],[temp2].[name] AS [sname] FROM (SELECT TOP {0} {3} FROM [{1}buysendpromotions] WHERE {2} ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                TableFields.BUYSEND_PROMOTIONS);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime],[temp1].[endtime],[temp1].[userranklower],[temp1].[state],[temp1].[name] AS [pname],[temp1].[type],[temp1].[buycount],[temp1].[sendcount],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}buysendpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.BUYSEND_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                    ...
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime],[temp1].[endtime],[temp1].[userranklower],[temp1].[state],[temp1].[name] AS [pname],[temp1].[type],[temp1].[buycount],[temp1].[sendcount],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}buysendpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.BUYSEND_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize,
        ...
        /// <param name="promotionTime">活动时间</param>
        /// <returns></returns>
        public string AdminGetBuySendPromotionListCondition(int storeId, string promotionName, string promotionTime)
        {
            StringBuilder condition = new StringBuilder();
        ...
        /// <param name="nowTime">当前时间</param>
        /// <returns></returns>
        public IDataReader GetBuySendPromotionList(int storeId, int pid, DateTime nowTime)
        {
            DbParameter[] parms = {
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByBuySendPmId(string pmIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}buysendpromotions] WHERE [pmid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               pmIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        /// <param name="nowTime">当前时间</param>
        /// <returns></returns>
        public IDataReader GetAllBuySendPromotion(int storeId, DateTime nowTime)
        {
            DbParameter[] parms = {
                                     GenerateInParam("@storeid", SqlDbType.Int, 4, storeId),
                                     GenerateInParam("@nowtime", SqlDbType.DateTime, 8, nowTime)
                                   };
            string commandText = string.Format("SELECT {1} FROM [{0}buysendpromotions] WHERE [storeid]=@storeid AND [state]=1 AND [starttime]<=@nowtime AND [endtime]>@nowtime",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.BUYSEND_PROMOTIONS);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByBuySendProductRecordId(string recordIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}buysendpromotions] WHERE [pmid] IN (SELECT DISTINCT [pmid] FROM [{0}buysendproducts] WHERE [recordid] IN ({1}))",
                                               RDBSHelper.RDBSTablePre,
                                               recordIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public void CreateGiftPromotion(GiftPromotionInfo giftPromotionInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@storeid", SqlDbType.Int, 4, giftPromotionInfo.StoreId),
            ...
            string commandText = string.Format("INSERT INTO [{0}giftpromotions]([pid],[storeid],[starttime1],[endtime1],[starttime2],[endtime2],[starttime3],[endtime3],[userranklower],[state],[name],[quotaupper]) VALUES(@pid,@storeid,@starttime1,@endtime1,@starttime2,@endtime2,@starttime3,@endtime3,@userranklower,@state,@name,@quotaupper)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public void UpdateGiftPromotion(GiftPromotionInfo giftPromotionInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}giftpromotions] SET [pid]=@pid,[storeid]=@storeid,[starttime1]=@starttime1,[endtime1]=@endtime1,[starttime2]=@starttime2,[endtime2]=@endtime2,[starttime3]=@starttime3,[endtime3]=@endtime3,[userranklower]=@userranklower,[state]=@state,[name]=@name,[quotaupper]=@quotaupper WHERE [pmid]=@pmid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        /// </summary>
        /// <param name="pmIdList">活动id</param>
        public void DeleteGiftPromotionById(string pmIdList)
        {
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public DataTable AdminGetGiftPromotionList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[pid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name],[temp1].[quotaupper],[temp2].[name] AS [pname],[temp2].[state] AS [pstate],[temp3].[name] AS [sname] FROM (SELECT TOP {0} [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[quotaupper] FROM [{1}giftpromotions] ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[pid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name],[temp1].[quotaupper],[temp2].[name] AS [pname],[temp2].[state] AS [pstate],[temp3].[name] AS [sname] FROM (SELECT TOP {0} [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[quotaupper] FROM [{1}giftpromotions] WHERE {2} ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            }
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[pid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name],[temp1].[quotaupper],[temp2].[name] AS [pname],[temp2].[state] AS [pstate],[temp3].[name] AS [sname] FROM (SELECT [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[quotaupper] FROM (SELECT TOP {0} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[quotaupper] FROM [{1}giftpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                pageSize * (pageNumber - 1) + 1,
                                                pageSize * pageNumber);
                    ...
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[pid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name],[temp1].[quotaupper],[temp2].[name] AS [pname],[temp2].[state] AS [pstate],[temp3].[name] AS [sname] FROM (SELECT [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[quotaupper] FROM (SELECT TOP {0} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [pmid],[pid],[storeid],[starttime1],[endtime1],[state],[name],[quotaupper] FROM [{1}giftpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                pageSize * (pageNumber - 1) + 1,
                                                pageSize * pageNumber,
        ...
        /// <param name="promotionTime">活动时间</param>
        /// <returns></returns>
        public string AdminGetGiftPromotionListCondition(int storeId, int pid, string promotionName, string promotionTime)
        {
            StringBuilder condition = new StringBuilder();
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByGiftPmId(string pmIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}giftpromotions] WHERE [pmid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               pmIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public IDataReader AdminGetExtGiftList(int pmId)
        {
            ...
            string commandText = string.Format("SELECT [temp1].[recordid],[temp1].[pmid],[temp1].[number],[temp2].[pid],[temp2].[psn],[temp2].[cateid],[temp2].[brandid],[temp2].[storeid],[temp2].[storecid],[temp2].[storestid],[temp2].[skugid],[temp2].[name],[temp2].[shopprice],[temp2].[marketprice],[temp2].[costprice],[temp2].[state],[temp2].[isbest],[temp2].[ishot],[temp2].[isnew],[temp2].[displayorder],[temp2].[weight],[temp2].[showimg] FROM (SELECT {1} FROM [{0}gifts] WHERE [pmid]=@pmid) AS [temp1] LEFT JOIN [{0}products] AS [temp2] ON [temp1].[giftid]=[temp2].[pid]",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.GIFTS);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        {
            DbParameter[] parms = {
                                        GenerateInParam("@storeid", SqlDbType.Int,4,suitPromotionInfo.StoreId),
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public DataTable AdminGetSuitPromotionList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name] AS [pname],[temp2].[name] AS [sname] FROM (SELECT TOP {0} [pmid],[storeid],[starttime1],[endtime1],[state],[name] FROM [{1}suitpromotions] ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name] AS [pname],[temp2].[name] AS [sname] FROM (SELECT TOP {0} [pmid],[storeid],[starttime1],[endtime1],[state],[name] FROM [{1}suitpromotions] WHERE {2} ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            }
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name] AS [pname],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}suitpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.SUIT_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize);
                    ...
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime1],[temp1].[endtime1],[temp1].[state],[temp1].[name] AS [pname],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}suitpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.SUIT_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize,
        ...
        /// <param name="promotionTime">活动时间</param>
        /// <returns></returns>
        public string AdminGetSuitPromotionListCondition(int storeId, int pid, string promotionName, string promotionTime)
        {
            StringBuilder condition = new StringBuilder();
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListBySuitPmId(string pmIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}suitpromotions] WHERE [pmid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               pmIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public IDataReader AdminGetExtSuitProductList(int pmId)
        {
            ...
            string commandText = string.Format("SELECT [temp1].[recordid],[temp1].[pmid],[temp1].[discount],[temp1].[number],[temp2].[pid],[temp2].[psn],[temp2].[cateid],[temp2].[brandid],[temp2].[storeid],[temp2].[storecid],[temp2].[storestid],[temp2].[skugid],[temp2].[name],[temp2].[shopprice],[temp2].[marketprice],[temp2].[costprice],[temp2].[state],[temp2].[isbest],[temp2].[ishot],[temp2].[isnew],[temp2].[displayorder],[temp2].[weight],[temp2].[showimg] FROM (SELECT [recordid],[pmid],[pid],[discount],[number] FROM [{0}suitproducts] WHERE [pmid]=@pmid) AS [temp1] LEFT JOIN [{0}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid]",
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        {
            string commandText = string.Format("SELECT [temp1].[recordid],[temp1].[pmid],[temp1].[discount],[temp1].[number],[temp2].[pid],[temp2].[psn],[temp2].[cateid],[temp2].[brandid],[temp2].[storeid],[temp2].[storecid],[temp2].[storestid],[temp2].[skugid],[temp2].[name],[temp2].[shopprice],[temp2].[marketprice],[temp2].[costprice],[temp2].[state],[temp2].[isbest],[temp2].[ishot],[temp2].[isnew],[temp2].[displayorder],[temp2].[weight],[temp2].[showimg] FROM (SELECT [recordid],[pmid],[pid],[discount],[number] FROM [{0}suitproducts] WHERE [pmid] IN ({1})) AS [temp1] LEFT JOIN [{0}products] AS [temp2] ON [temp1].[pid]=[temp2].[pid]",
                                                RDBSHelper.RDBSTablePre,
                                                pmIdList);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        public IDataReader AdminGetFullSendPromotionById(int pmId)
        {
            ...
            string commandText = string.Format("SELECT {1} FROM [{0}fullsendpromotions] WHERE [pmid]=@pmid",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.FULLSEND_PROMOTIONS);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        {
            DbParameter[] parms = {
                                        GenerateInParam("@storeid", SqlDbType.Int,4,fullSendPromotionInfo.StoreId),
            ...
            string commandText = string.Format("INSERT INTO [{0}fullsendpromotions]([storeid],[starttime],[endtime],[userranklower],[state],[name],[limitmoney],[addmoney]) VALUES(@storeid,@starttime,@endtime,@userranklower,@state,@name,@limitmoney,@addmoney)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public void UpdateFullSendPromotion(FullSendPromotionInfo fullSendPromotionInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}fullsendpromotions] SET [storeid]=@storeid,[starttime]=@starttime,[endtime]=@endtime,[userranklower]=@userranklower,[state]=@state,[name]=@name,[limitmoney]=@limitmoney,[addmoney]=@addmoney WHERE [pmid]=@pmid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public DataTable AdminGetFullSendPromotionList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[name],[temp1].[limitmoney],[temp1].[addmoney],[temp2].[name] AS [sname] FROM (SELECT TOP {0} [pmid],[storeid],[starttime],[endtime],[state],[name],[limitmoney],[addmoney] FROM [{1}fullsendpromotions] ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[name],[temp1].[limitmoney],[temp1].[addmoney],[temp2].[name] AS [sname] FROM (SELECT TOP {0} [pmid],[storeid],[starttime],[endtime],[state],[name],[limitmoney],[addmoney] FROM [{1}fullsendpromotions] WHERE {2} ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            }
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[name],[temp1].[limitmoney],[temp1].[addmoney],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}fullsendpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.FULLSEND_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize);
                    ...
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[name],[temp1].[limitmoney],[temp1].[addmoney],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}fullsendpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.FULLSEND_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize,
        ...
        /// <param name="promotionTime">活动时间</param>
        /// <returns></returns>
        public string AdminGetFullSendPromotionListCondition(int storeId, string promotionName, string promotionTime)
        {
            StringBuilder condition = new StringBuilder();
        ...
        /// <param name="nowTime">当前时间</param>
        /// <returns></returns>
        public IDataReader GetFullSendPromotionByStoreIdAndPidAndTime(int storeId, int pid, DateTime nowTime)
        {
            DbParameter[] parms = {
                                    GenerateInParam("@storeid", SqlDbType.Int, 4, storeId),  
                                    GenerateInParam("@pid", SqlDbType.Int, 4, pid),  
            ...
            return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,
                                            string.Format("{0}getfullsendpromotionbystoreidandpidandtime", RDBSHelper.RDBSTablePre),
                                            parms);
        }
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByFullSendPmId(string pmIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}fullsendpromotions] WHERE [pmid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               pmIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public IDataReader GetFullSendMainProductList(int pageSize, int pageNumber, int pmId, int startPrice, int endPrice, int sortColumn, int sortDirection)
        {
            ...
            if (pageNumber == 1)
            {
                ...
                commandText.AppendFormat(" WHERE [p].[pid] IN (SELECT [pid] FROM [{0}fullsendproducts] WHERE [pmid]={1} AND [type]=0)", RDBSHelper.RDBSTablePre, pmId);
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByFullSendProductRecordId(string recordIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}fullsendpromotions] WHERE [pmid] IN (SELECT DISTINCT [pmid] FROM [{0}fullsendproducts] WHERE [recordid] IN ({1}))",
                                               RDBSHelper.RDBSTablePre,
                                               recordIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public IDataReader AdminGetFullCutPromotionById(int pmId)
        {
            ...
            string commandText = string.Format("SELECT {1} FROM [{0}fullcutpromotions] WHERE [pmid]=@pmid",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.FULLCUT_PROMOTIONS);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public DataTable AdminGetFullCutPromotionList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[type],[temp1].[starttime],[temp1].[endtime],[temp1].[userranklower],[temp1].[state],[temp1].[name],[temp1].[limitmoney1],[temp1].[cutmoney1],[temp1].[limitmoney2],[temp1].[cutmoney2],[temp1].[limitmoney3],[temp1].[cutmoney3],[temp2].[name] AS [sname] FROM (SELECT TOP {0} {2} FROM [{1}fullcutpromotions] ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.FULLCUT_PROMOTIONS);

                else
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[type],[temp1].[starttime],[temp1].[endtime],[temp1].[userranklower],[temp1].[state],[temp1].[name],[temp1].[limitmoney1],[temp1].[cutmoney1],[temp1].[limitmoney2],[temp1].[cutmoney2],[temp1].[limitmoney3],[temp1].[cutmoney3],[temp2].[name] AS [sname] FROM (SELECT TOP {0} {3} FROM [{1}fullcutpromotions] WHERE {2} ORDER BY [pmid] DESC) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                TableFields.FULLCUT_PROMOTIONS);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[type],[temp1].[starttime],[temp1].[endtime],[temp1].[userranklower],[temp1].[state],[temp1].[name],[temp1].[limitmoney1],[temp1].[cutmoney1],[temp1].[limitmoney2],[temp1].[cutmoney2],[temp1].[limitmoney3],[temp1].[cutmoney3],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}fullcutpromotions]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.FULLCUT_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize);
                    ...
                    commandText = string.Format("SELECT [temp1].[pmid],[temp1].[storeid],[temp1].[type],[temp1].[starttime],[temp1].[endtime],[temp1].[userranklower],[temp1].[state],[temp1].[name],[temp1].[limitmoney1],[temp1].[cutmoney1],[temp1].[limitmoney2],[temp1].[cutmoney2],[temp1].[limitmoney3],[temp1].[cutmoney3],[temp2].[name] AS [sname] FROM (SELECT {0} FROM (SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY [pmid] DESC) AS [rowid],{0} FROM [{1}fullcutpromotions] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}stores] AS [temp2] ON [temp1].[storeid]=[temp2].[storeid]",
                                                TableFields.FULLCUT_PROMOTIONS,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize + 1,
                                                pageNumber * pageSize,
        ...
        /// <param name="promotionTime">活动时间</param>
        /// <returns></returns>
        public string AdminGetFullCutPromotionListCondition(int storeId, string promotionName, string promotionTime)
        {
            StringBuilder condition = new StringBuilder();
        ...
        /// <param name="nowTime">当前时间</param>
        /// <returns></returns>
        public IDataReader GetFullCutPromotionByStoreIdAndPidAndTime(int storeId, int pid, DateTime nowTime)
        {
            DbParameter[] parms = {
                                    GenerateInParam("@storeid", SqlDbType.Int, 4, storeId),  
                                    GenerateInParam("@pid", SqlDbType.Int, 4, pid),  
            ...
            return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,
                                            string.Format("{0}getfullcutpromotionbystoreidandpidandtime", RDBSHelper.RDBSTablePre),
                                            parms);
        }
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByFullCutPmId(string pmIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}fullcutpromotions] WHERE [pmid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               pmIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        /// <param name="nowTime">当前时间</param>
        /// <returns></returns>
        public IDataReader GetAllFullCutPromotion(int storeId, DateTime nowTime)
        {
            DbParameter[] parms = {
                                     GenerateInParam("@storeid", SqlDbType.Int, 4, storeId),
                                     GenerateInParam("@nowtime", SqlDbType.DateTime, 8, nowTime)    
                                   };
            string commandText = string.Format("SELECT {1} FROM [{0}fullcutpromotions] WHERE [storeid]=@storeid AND [state]=1 AND [starttime]<=@nowtime AND [endtime]>@nowtime",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.FULLCUT_PROMOTIONS);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByFullCutProductRecordId(string recordIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}fullcutpromotions] WHERE [pmid] IN (SELECT DISTINCT [pmid] FROM [{0}fullcutproducts] WHERE [recordid] IN ({1}))",
                                               RDBSHelper.RDBSTablePre,
                                               recordIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        /// <returns></returns>
        public string AdminGetCouponTypeListCondition(int storeId, int type, string couponTypeName)
        {
            StringBuilder condition = new StringBuilder();
            ...
            if (!string.IsNullOrWhiteSpace(couponTypeName) && SecureHelper.IsSafeSqlString(couponTypeName))
        ...
        /// <param name="storeId">店铺id</param>
        /// <returns></returns>
        public IDataReader GetSendingPromotionCouponTypeList(int storeId)
        {
            DbParameter[] parms = {
                                        GenerateInParam("@storeid", SqlDbType.Int, 4, storeId)    
                                    };
            string commandText = string.Format("SELECT {0} FROM [{1}coupontypes] WHERE [storeId]=@storeid AND [sendmode]=2 AND [state]=1",
                                                TableFields.COUPON_TYPES,
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        /// <param name="nowTime">当前时间</param>
        /// <returns></returns>
        public IDataReader GetSendingCouponTypeList(int storeId, DateTime nowTime)
        {
            DbParameter[] parms = {
                                        GenerateInParam("@storeid", SqlDbType.Int, 4, storeId),
                                        GenerateInParam("@nowtime", SqlDbType.DateTime, 8, nowTime)    
        ...
        /// <param name="nowTime">当前时间</param>
        /// <returns></returns>
        public IDataReader GetUsingCouponTypeList(int storeId, DateTime nowTime)
        {
            DbParameter[] parms = {
                                        GenerateInParam("@storeid", SqlDbType.Int, 4, storeId),
                                        GenerateInParam("@nowtime", SqlDbType.DateTime, 8, nowTime)    
        ...
        /// <param name="couponTypeIdList">优惠劵类型id列表</param>
        /// <returns></returns>
        public DataTable GetStoreIdListByCouponTypeId(string couponTypeIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}coupontypes] WHERE [coupontypeid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               couponTypeIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public void CreateCoupon(CouponInfo couponInfo)
        {
            DbParameter[] parms = {
                                    ...
                                    GenerateInParam("@storeid", SqlDbType.Int,4,couponInfo.StoreId),
        ...
        /// <param name="couponTypeId">优惠劵类型id</param>
        /// <returns></returns>
        public string AdminGetCouponListCondition(string sn, int uid, int couponTypeId)
        {
            StringBuilder condition = new StringBuilder();

            condition.AppendFormat(" [coupontypeid]={0}", couponTypeId);
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByCouponId(string couponIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}coupons] WHERE [couponid] IN ({1})",
                                               RDBSHelper.RDBSTablePre,
                                               couponIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        /// <returns></returns>
        public DataTable GetStoreIdListByCouponProductRecordId(string recordIdList)
        {
            string commandText = string.Format("SELECT DISTINCT [storeid] FROM [{0}coupontypes] WHERE [coupontypeid] IN (SELECT DISTINCT [coupontypeid] FROM [{0}couponproducts] WHERE [recordid] IN ({1}))",
                                               RDBSHelper.RDBSTablePre,
                                               recordIdList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
